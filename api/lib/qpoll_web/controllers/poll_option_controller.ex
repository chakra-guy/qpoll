defmodule QpollWeb.PollOptionController do
  ...
  def index(conn, _params) do
    poll_options = Polls.list_poll_options()
    render(conn, "index.json", poll_options: poll_options)
  end

  def create(conn, %{"poll_option" => poll_option_params}) do
    with {:ok, %PollOption{} = poll_option} <- Polls.create_poll_option(poll_option_params) do
      conn
      |> put_status(:created)
      |> put_resp_header("location", Routes.poll_option_path(conn, :show, poll_option))
  ...
  def update(conn, %{"id" => id, "poll_option" => poll_option_params}) do
    ...
    with {:ok, %PollOption{} = poll_option} <- Polls.update_poll_option(poll_option, poll_option_params) do
      render(conn, "show.json", poll_option: poll_option)
    end
  end
